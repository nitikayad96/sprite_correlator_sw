...
if __name__ == '__main__':
    ...
    p.add_option('-p', '--plot', dest='plot', action='store_true', default=True,
    ...
    for ifeng,feng in enumerate(corr.fengs):
        ...
        if load_new:
            ...
            print 'Snapping data from ANT: %d, BAND: %s'%(feng.ant,feng.band)
            #d += feng.get_spectra()
            mcnt_old = xeng.read_uint('mcnt_lsb')
            mcnt = xeng.read_uint('mcnt_lsb')
            while mcnt == mcnt_old:
                mcnt = xeng.read_uint('mcnt_lsb')
            specs = corr.snap_corr_wide(wait=False,combine_complex=False)
            mcnt_old = mcnt
            while mcnt == mcnt_old:
                mcnt = xeng.read_uint('mcnt_lsb')
            specs = corr.snap_corr_wide(wait=False,combine_complex=False)            
